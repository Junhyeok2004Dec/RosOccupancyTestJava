...
public class RayCasting extends JFrame {
	...
	public void update() {
		...
		centerX = this.gridInstance.getRectangleModel().getPositionX() + (0.5)*this.gridInstance.getRectangleModel().getScale()[0];
		centerY = this.gridInstance.getRectangleModel().getPositionY() + (0.5)*this.gridInstance.getRectangleModel().getScale()[1];
	...
	private class Ray
	{
		...
		public Ray(int startX, int startY, int angle, RayCasting rayCasting, boolean wasClicked) {
			...
			dt = this.rayCasting.gridInstance.simulator.getDt();
		}
